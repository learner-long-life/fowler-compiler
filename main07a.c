...
int main() {
   ...
   int n, width;
   double dist, dist2, temp_dist, x, y;
   Matrix G;
   ...
   product=(int *)calloc(MEDIUM, sizeof(int));
...
//      G.z22.x=cos(Pi*numerator/denominator);
//      G.z22.y=sin(Pi*numerator/denominator);
      G.z22.x=cos(x);
      G.z22.y=sin(x);
   }
   fclose(in);
